...
const ActiveSwitch = ({ checked, onChange }) => (
    <label className="flex items-center gap-3 cursor-pointer select-none">
        <div className="relative">
            ...
            />
            <div
                className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition ${checked ? "translate-x-5" : ""}`}
            />
        </div>
...
export default function Edit({ hotel }) {
    ...
    const selectStyles = {
        control: (base, state) => ({
            ...
            borderColor: frontendErrors.city
                ? "#ef4444"
                : state.isFocused
                  ? "var(--app-primary)"
                  : "#d1d5db",
    ...
    const handleCityChange = (option) => {
        ...
        form.setData({
            ...
            city,
            distance_from_kaaba:
                city === "مكة" ? form.data.distance_from_kaaba : "",
            distance_from_nabawi:
                city === "المدينة المنورة"
                    ? form.data.distance_from_nabawi
                    : "",
        });
    ...
    const validate = () => {
        ...
        if (form.data.city === "مكة" && !form.data.distance_from_kaaba)
            errors.distance_from_kaaba = "المسافة من الكعبة مطلوبة";
        if (
            form.data.city === "المدينة المنورة" &&
            !form.data.distance_from_nabawi
        )
            errors.distance_from_nabawi = "المسافة من النبوي مطلوبة";
        if (!form.data.stars) errors.stars = "اختر تقييم الفندق";
        if (!form.data.address_location.trim())
            errors.address_location = "العنوان مطلوب";
        if (!form.data.image && !hotel.image_path) {
            errors.image = "يرجى رفع صورة";
        }
...
//     }
        ...
        form.post(route("hotels.update", hotel.id), {
            data: formData,
            onSuccess: () => {
            ...
            onError: () => {
                toast.error(
                    "حدثت أخطاء في الإدخال. يرجى التحقق والمحاولة مرة أخرى.",
                );
            },
        });
    };
        ...
        <AuthenticatedLayout>
            ...
            >
                ...
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-bold">تعديل فندق</h1>
                </div>
                ...
                <form onSubmit={submit} className="card p-6 space-y-6 w-full">
                    <div>
                        ...
                        />
                        <InputError
                            message={frontendErrors.name || form.errors.name}
                        />
                    </div>
                    ...
                    <div>
                        ...
                        />
                        <InputError
                            message={frontendErrors.desc || form.errors.desc}
                        />
                    </div>
                    ...
                    <div>
                        ...
                        />
                        <InputError
                            message={frontendErrors.city || form.errors.city}
                        />
                    </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setFrontendErrors((prev) => ({
                                        ...prev,
                                        distance_from_kaaba: null,
                                    }));
                                }}
                            />
                            <InputError
                                message={
                                    frontendErrors.distance_from_kaaba ||
                                    form.errors.distance_from_kaaba
                                }
                            />
                        </div>
                        <div>
                            <label className="label">
                                المسافة من النبوي (متر)
                            </label>
                            <input
                                type="number"
                                min={0}
                            ...
                            <InputError
                                message={
                                    frontendErrors.distance_from_nabawi ||
                                    form.errors.distance_from_nabawi
                                }
                            />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        />
                        <InputError
                            message={frontendErrors.stars || form.errors.stars}
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <InputError
                            message={
                                frontendErrors.address_location ||
                                form.errors.address_location
                            }
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex flex-col mt-2 gap-2">
                            <button
                                ...
                                onClick={() =>
                                    document
                                        .getElementById("hotel-image-input")
                                        .click()
                                }
                        ...
                        <InputError
                            message={frontendErrors.image || form.errors.image}
                        />
                    </div>
