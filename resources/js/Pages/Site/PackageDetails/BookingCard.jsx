...
export default function BookingCard({ data }) {
    ...
    const startDateHijri = data?.start_date
        ? formatHijriDate(data.start_date)
        : "غير متوفر";
    const endDateHijri = data?.end_date
        ? formatHijriDate(data.end_date)
        : "غير متوفر";
                ...
                {data.prices && data.prices.length > 0 && (
                    <>
                        <button
                            ...
                            className="w-full flex items-center justify-between text-green-700 text-decoration-underline text-sm mb-4"
                        >
                            <span>
                                {showPrices
                                    ? "سعر الفرد في الغرفة"
                ...
                <AnimatePresence>
                    {showPrices && (
                        <motion.div
                            ...
                            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-1 gap-3 mb-4"
                        >
                            {data.prices && data.prices.length > 0 ? (
